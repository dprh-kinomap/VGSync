...
def _is_verbose():
    ...
    try:
        ...
        if QSettings("VGSync","VGSync").value("app/debug", False, type=bool):
            return True
    except Exception:
        pass
    return False
...
if platform.system() == "Windows":
    try:
        ...
        if hwnd:
            ...
            k32.SetConsoleTitleW("VGSync DEBUG Konsole" if DEBUG else "VGSync Konsole - Don’t Close")
            ...
            if not DEBUG:
                ...
                if k32.GetConsoleScreenBufferInfo(hStdOut, ctypes.byref(csbi)):
                    ...
                    force_print("====   VGSync DEBUG Konsole    ====")
...
from config import (
    ...
    migrate_settings_from_kvrouite,
)
